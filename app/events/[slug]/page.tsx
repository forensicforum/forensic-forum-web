...
export default async function EventDetailsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="flex-grow max-w-4xl mx-auto px-6 py-16 w-full space-y-10 relative">
      ...
      {/* The updated image container that preserves the flyer's natural aspect ratio */}
      {event.coverImage && (
        <div className="w-full rounded-xl overflow-hidden mt-8 border border-gray-800 bg-[#0a0d10] flex justify-center">
          <img 
            src={urlFor(event.coverImage).width(1200).url()} 
            alt={event.title} 
